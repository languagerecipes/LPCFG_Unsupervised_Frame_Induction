 ...
 */
public class CYKParsingCallableAllParsePruning implements Callable<Collection<ParseDataCompact>> {

    final private Collection<Fragment> fcp;
    final private RuleMaps theRuleMap;
    final private double targetedNBestPortion;
    final private RulesCounts rcForTopNParses;
    //int id;

    public CYKParsingCallableAllParsePruning(Collection<Fragment> fcp, RuleMaps theRuleMap, double targetedNBestPortion) {
        this.fcp = fcp;
        this.theRuleMap = theRuleMap;
        this.targetedNBestPortion = targetedNBestPortion;
            ...
            try {
                ...
                if (validParsesSortByProb.size() > 0) { // this must be the case otherwise there are some erorrs
                    ...
                    if (totalParseForThis > 1) {
                        int s = (int) Math.floor(totalParseForThis * targetedNBestPortion);
...
//                    probSum.add(log);
                    ...
                    bestParseFrame.setLikelihood(llForBestParse);
