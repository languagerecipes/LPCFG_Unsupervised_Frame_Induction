...
public class MergeManyPartialSpanFilter {
    ...
    int maxNumberOfMergesForAGivenSet = 5;
    double thresholdSim;
    ...
    public MergeManyPartialSpanFilter(
...
//        this.evList = evList;
    ...
    public Collection<ParseFrame> mergeStart(
        ...
        Map<String, Collection<Fragment>> initialFragmentClustersFullSize
                = HelperFragmentMethods.partitionFragmentsByClustersToMapListPartialSpan(
                        fgAll, initparseframes);
        this.fgAll = HelperFragmentMethods.flattenFragmentCollections(initialFragmentClustersFullSize.values());
        Settings settins = new Settings();
        if (logDetailPerformance) {
            evList.evaluationFromParsed(999,
                    initparseframes);
            System.err.println(" Starting point >> " + evList.getLastEvaluation().toStringShort(" "));
        }
        ...
        Map<String, Collection<Fragment>> partitionNew = HelperFragmentMethods.partitionFragmentsByClustersToMapListPartialSpan(fgAll, initparseframes);
        Collection<Fragment> remove = partitionNew.remove("-1"); // this the unknown cases
        if(remove!=null){
        System.err.println("Removed instances that not appeared in training set so far " + remove.size());}
        Collection<Fragment> flattenFragmentCollections = HelperFragmentMethods.flattenFragmentCollections(partitionNew.values());
        genGrmFromClusters.genRules(flattenFragmentCollections, initparseframes);
        theRuleMap = genGrmFromClusters.getTheRuleMap();
        System.err.println("Rule map is generated!");
        int counter = 0;
        int countMerge = 0;
        while (thresholdSim - delta > minSim) {
            ...
            if (logDetailPerformance) {
                ...
                System.err.println(this.thresholdSim + ">> " + evList.getLastEvaluation().toStringShort(" "));
            }
        }
        System.err.println(counter++ + "> " + thresholdSim + " >");
